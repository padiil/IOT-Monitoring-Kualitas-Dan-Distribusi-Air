...
function App() {
  ...
  // WebSocket logic to receive sensor data
  useEffect(() => {
    ws.current = new WebSocket(WS_URL); // Assign WebSocket instance to `ws.current`
    ...
    ws.current.onmessage = (event) => {
      try {
        ...
        const activeData = Object.keys(sliderData).reduce((acc, key) => {
          acc[key] = useServer[key]
            ? newData[key] || sliderData[key]
            : sliderData[key];
          return acc;
        }, {});
        ...
        setIPjData((prevData) => {
          const newData = [
            ...prevData,
            { time: new Date().toLocaleTimeString(), IPj },
          ];
